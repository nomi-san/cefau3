...
Global $__OnAddressChange__param			= 'ptr;ptr;ptr;str', 					$__OnAddressChange__ret				= 'none'
...
; If cache path is Null, it will set to default path ' ./cache '.
Func Cef_Init($hInstance = Null, $sCachePath = Null)
	If $sCachePath = Default or (Not $sCachePath) Then $sCachePath = Null
	Local $aRet = DllCall($__handle__Cefau3, 'ptr', 'Cef_Init', 'handle', $hInstance, 'str', $sCachePath)
	If @error Then Return 0
	Return $aRet[0]
EndFunc

; Close main browser on handler.
Func Cef_Close($Cef)
	DllCall($__handle__Cefau3, 'none', 'Cef_Close', 'ptr', $Cef)
EndFunc
