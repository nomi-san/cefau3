...
typedef void(__stdcall * _OnAddressChange_)(Cefau3_Handler*, CefBrowser*, CefFrame*, const char*);
void Cefau3_Handler::OnAddressChange(CefRefPtr<CefBrowser> browser,
	CefRefPtr<CefFrame> frame,
	const CefString& url)
{
	if (this->_OnAddressChange)
		((_OnAddressChange_)this->_OnAddressChange)(this, browser, frame, url.ToString().c_str());
}
	...
	const std::vector<CefString>& icon_urls)
{
	if (this->_OnFaviconURLChange) {
		...
		((_OnFaviconURLChange_)this->_OnFaviconURLChange)(this, browser, filter.c_str());
	}
}
